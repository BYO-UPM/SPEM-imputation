...
def Refinement_nan_pipeline(original_data, imputation_method, Target, device, oDATA):
    ...
    elif imputation_method == 'csdi':
        imputed_results = imputation_csdi(downsampled_data)
...
def calculate_metrics(predicted_signals, original_signals, indicating_mask):
   ...
   # Modify MRE calculation to match csdi
...
def imputation_csdi(downsampled_data):
    n_folds = 10
    ...
    for fold_index in range(1, n_folds + 1):
        ...
        model_path = f"csdi_weights/csdi_all_artificial_4_kfolds_{fold_index}.pypots"
        csdi.load(model_path)

        dataset = {"X": expanded_data}
        imputed_results = csdi.impute(dataset)
...
csdi_interpolated, csdi_refined, csdi_down = Refinement_nan_pipeline(oDATA_AR, imputation_method, Target, device, oDATA)
...
#csdi tiene kfolds del csdi y kfolds del Autoencoder
# Procesar cada fila (primer dimensión)
aggregated_metrics = {
        'MAE_mean': [],
        'MRE_mean': [],
...
num_signals = len(csdi_refined)
for i in range(num_signals):
    # Extraer la fila actual

    imputed_slice = csdi_refined[i]
...
# Calcular la media de las métricas
metrics_csdi_refined = {key: np.mean(values) if values else None for key, values in aggregated_metrics.items()}
...
metrics_csdi_interpolated = compute_all_metrics_kfolds(csdi_interpolated, np.nan_to_num(oDATA), indicating_mask_2)
...
metrics_csdi_down = compute_all_metrics_kfolds(np.array(csdi_down), np.nan_to_num(down_oDATA), indicating_mask_1)
...
print("Computed Metrics csdi Refined:")
for key, value in metrics_csdi_refined.items():
    print(f"{key}: {value:.4f}")
print('-' * 50)
...
print("Computed Metrics csdi Interpolated:")
for key, value in metrics_csdi_interpolated.items():
    print(f"{key}: {value:.4f}")
print('-' * 50)
...
print("Computed Metrics csdi Downsampled:")
for key, value in metrics_csdi_down.items():
    print(f"{key}: {value:.4f}")
print('-' * 50)
...
imputation_method = 'csdi'
csdi_interpolated, csdi_refined, csdi_down = Refinement_nan_pipeline(oDATA_AR1, imputation_method, Target, device, oDATA)
...
metrics_csdi_interpolated = compute_all_metrics_kfolds(csdi_interpolated, np.nan_to_num(oDATA), indicating_mask_region)
